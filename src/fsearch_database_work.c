...
FsearchDatabaseWork *
fsearch_database_work_new_sort(guint view_id, FsearchDatabaseIndexProperty sort_order, FsearchSortType sort_type) {
    FsearchDatabaseWork *work = work_new();
...
fsearch_database_work_search_get_sort_type(FsearchDatabaseWork *work) {
    g_return_val_if_fail(work, FSEARCH_SORT_ASCENDING);
    g_return_val_if_fail(work->kind == FSEARCH_DATABASE_WORK_SEARCH, FSEARCH_SORT_ASCENDING);
...
fsearch_database_work_sort_get_sort_type(FsearchDatabaseWork *work) {
    g_return_val_if_fail(work, FSEARCH_SORT_ASCENDING);
    g_return_val_if_fail(work->kind == FSEARCH_DATABASE_WORK_SORT, FSEARCH_SORT_ASCENDING);
    return work->sort_type;
}
